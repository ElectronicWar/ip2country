...
;#                                                                                        #
;#  Version: ver.1.02  -  released on 20 Sept 2010                                        #
...
;#                                                                                        #
;# //load -rs1 $+(",$nofile($mircexe),ip2country\ip2country.ver.1.02.mrc,")               #
;#                                                                                        #
;#  Otherwise, open the Remote Script Area ( Tools | Script Editor) and                   #
;#  click on File | Load, and find the file ip2country.ver.1.02.mrc                       #
;#                                                                                        #
...
;#  To the "helpers" on #mIRCscripting Undernet.                                          #
...
ALIAS ip2c  {
  ...
  ; To speed up processing - if the IP is valid - jump straight to the IP verify
  ...
  ; Check if the IP is masked for Undernet +x mode
  ...
  if ($group(#ip2c.2nd.Level.Domains) == on) {
    ; To speed up processing - if the IP is a KNOWN 2nd Level Domain
  ...
  ; Check if the IP is a TLD - Top Level Domain
  ...
  :host2ip.found
  ;  Check if the IP number entered IS a Country Code IP
  ...
  ;  Check if the IP number entered is NOT valid numerical IP
...
ALIAS ip2c.lookup.heart {
  ...
  !if (%ip >= $1) { !if %ip <= $2 { !var %f = FOUND | !goto F } }
  !if (%lr == %lf) { !var %f = NOTFOUND | !goto F }
  !if (%ip > $1) { !var %lf = %lr, %lr = $round($calc((%lf + %ll) / 2),0) | !goto L }
  !else { !var %ll = %lr, %lr = $int($calc((%lf + %ll) / 2)) | !goto L }
  :F
...
ALIAS ip2c.Not.Found.Log.to.File {
  tokenize 32 $1-
  var %longip = $1, %line.read = $2
  write ip2c.ip.not.found.txt $longip(%longip) : %longip : Closest Line %line.read : $asctime($ctime,HH:nn:ss tt - ddd dd/mmm/yy) - $ctime
}
...
on *:TEXT:!ip *:%ip2c.channels: { 
  ...
  if (%chan) {
    ...
    if (%result != $null) {
      ...
      ;if ($true) {
      var %ip.comchans = $iif($group(#ip2c.ComChan) == ON,$ip2c.ial.check($2),)
      ;var %ip.comchans = $ip2c.ial.check($2)
      ;if (%ip.comchans) { msg %chan IP: $2 is %ip.comchans }
      ;}
      msg %chan IP: $2 is from %country ( $+ %cc $+ ) $iif(%ip.comchans,: Nick with this IP is %ip.comchans,)

    }
    else { msg %chan IP: $2 : Country not found }
  }
...
ctcp *:DCC CHAT: ip2c.echo.msg  $nick is $1 $2 $+ ing you from IP No. $longip($4) Port: $5  $+($chr(40),$ip2c($longip($4)).country,$chr(41)) 
ctcp *:DCC: ip2c.echo.msg  $nick is $1 $2 $+ ing you from IP No. $longip($4) Port: $5  $+($chr(40),$ip2c($longip($4)).country,$chr(41)) 
...
ALIAS ip2c.verify.ip.TLD {
  tokenize 32 $1-
  var %ip2c.tld.check = $gettok($1,$gettok($1,0,46),46)
  var %ip2c.TLDs = com net org edu gov mil arpa info biz mobi tel pro asia name aero cat coop jobs museum travel int
...
ALIAS ip2c.channel.info {
  ...
  while (%line <= %nicks) {
    ...
    var %cc = $ip2c(%addy).cc
    aline %window $iif(%cc,%cc ..,.. .. . ) $nick(%ch,%line).pnick  $chr(9) $iif(%comchan,%comchan, ) $chr(9) %addy  
    inc %line
  }
}
...
menu channel,nicklist,status,query,menubar {
  ...
  ...Website isn't ready yet, Use the Forum:run http://www.much.net/forums/index.php?c=11
...
ALIAS ip.m.switch.ON.recommended  { ip2c.echo.menu $ip2c.g Switching RECOMMENDED $bold(ON)  | linesep %ip2c.echo.result.place | ip.m.t.q.c.n.OFF.all  | ip.m.j.p.q.ON.all  | ip.m.o.i.a.ON.y.OFF.rec | .enable #ip2c.DNS #ip2c.2nd.Level.Domains #ip2c.ComChan | .disable #ip2c.country #ip2c.short.cc #ip2c.NOT-FOUND.log #ip2c.NOT.Found.echo #ip2c.Long.IP.Found.Result #ip2c.check.remote.status #ip2c.DNS.long  | linesep %ip2c.echo.result.place   }
...
;### This alias is Still under development even on release of ver 1.02
ALIAS ip2c.var { 
...
;
;### NORMAL with/add - [12:30] * nickname (user@host) has joined #channel
...
on  ^*:part:#: { 
  ...
  if (%ip2c.where.show.part) {
    ...
    if (%ip2c.join.part.short) { 
      var %country.SHOW.chan = $iif($group(#ip2c.short.cc) == on,%cc.join.chan,%country.SHOW.chan)
      echo %ip2c.part.echo * $iif(%ip2c.part.show.chan,Parts $chan $+ :, Parts:) $nick $iif(%ip2c.part.address,%ip2c.part.address,) $iif(%country.SHOW.chan,$+($chr(40),%country.SHOW.chan,$chr(41)),) $iif(%com.chan.all,$+($chr(40),%com.chan.all,$chr(41)),) 
    }
    else { 
      echo %ip2c.part.echo * $nick $iif(%ip2c.part.address,%ip2c.part.address,) $iif(%country.SHOW.chan, from %country.SHOW.chan,) $iif(%com.chan.all,also in %com.chan.all,) has left $chan 
    }   
  }
...
ALIAS ip2c.IAL.update {
  ...
  while $gettok($2-,%range,32) {
    ...
    var %names.list = $remove($remove(%names.list,+),@)

    ; enable #ip2c.userhost.reply.blocked
    var %ip2c.timer.num = $timer(0)

    find.last.timer -ip2c.ial.*
...
ALIAS ip2c.read.mirc.ini.join.part.short {
  ...
  else { ip2c.echo.NotFound Joins/Parts display has NO RESULT - programming mistake in ip2c.read.mirc.ini.join.part.short | return $false  }
}
...
ALIAS ip2c.read.mirc.ini.join.part.address.shown {
  ...
  else { ip2c.echo.NotFound User Address display has NO RESULT - programming error in ALIAS ip2c.read.mirc.ini.join.part.address.shown | return $false  }
}
...
ALIAS ip2c.read.mirc.ini.where.channel.join {
  ;### See WHERE the JOINS are shown for this channel

  ; First try and set the variable to the #Channel option if it exists. If not, then to the default.
  ; Joins/Parts for specific channel - events - #chan/default - pos 1/2
  if ($ini($mircini,events,$1) != 0) { 
    var %ip2c.mirc.ini.events = $readini($mircini,n,events,$1)
    var %ip2c.mirc.ini.events.pos1 =  $gettok(%ip2c.mirc.ini.events,1,44)
  }
  if ((!%ip2c.mirc.ini.events.pos1) || (%ip2c.mirc.ini.events.pos1 == 0)) { 
    var %ip2c.mirc.ini.events = $readini($mircini,n,events,default)
    var %ip2c.mirc.ini.events.pos1 =  $calc($gettok(%ip2c.mirc.ini.events,1,44) + 1)
  }

  ;### This section to see WHERE the EVENTS are shown
  ; Joins pos 01 - 0=Channel,1=Status,2=Hide 

  if (%ip2c.mirc.ini.events.pos1 == 1) { 
    return CHANNEL 
  }
  elseif (%ip2c.mirc.ini.events.pos1 == 2) { 
    return STATUS 
  }
  elseif (%ip2c.mirc.ini.events.pos1 == 3) { 
    return $false  
  }
  else { ip2c.echo.NotFound Joins display place has NO RESULT | return $false  }
}

;### Reads from the mirc.ini file to see if WHERE the PART display is supposed to go
; Needs to receive the $chan name
ALIAS -l ip2c.read.mirc.ini.where.channel.part {
  ;### See WHERE the PARTS are shown for this channel

  ; First try and set the variable to the #Channel option if it exists. If not, then to the default.
  ; Parts for specific channel - events - #chan/default - pos 2

  if ($ini($mircini,events,$1) != 0) { 
    var %ip2c.mirc.ini.events = $readini($mircini,n,events,$1)
    var %ip2c.mirc.ini.events.pos2 =  $gettok(%ip2c.mirc.ini.events,2,44)
  }

  if ((!%ip2c.mirc.ini.events.pos2) || (%ip2c.mirc.ini.events.pos2 == 0)) { 
    var %ip2c.mirc.ini.events = $readini($mircini,n,events,default)
    var %ip2c.mirc.ini.events.pos2 =  $calc($gettok(%ip2c.mirc.ini.events,2,44) + 1)
  }

  ;### This section to see WHERE the EVENTS are shown
  ;Parts pos 02 - 0=Channel,1=Status,2=Hide 

  if (%ip2c.mirc.ini.events.pos2 == 1) { 
    return CHANNEL 
  }
  elseif (%ip2c.mirc.ini.events.pos2 == 2) { 
    return STATUS  
  }
  elseif (%ip2c.mirc.ini.events.pos2 == 3) { 
    ;ip2c.echo.NotFound Parts for $1 shown in NOT SHOW 
    return $false 
  }
  else { ip2c.echo.NotFound Parts display place has NO RESULT | return $false  }
}
...
ALIAS ip2c.read.mirc.ini.quit {
  ...
  ; First try and set the variable to the #Channel option if it exists. If not, then to the default.
  ; Quits for specific channel - events - #chan/default - pos 3
  if ($ini($mircini,events,$1) != 0) { 
    var %ip2c.mirc.ini.events = $readini($mircini,n,events,$1)
    var %ip2c.mirc.ini.events.pos3 =  $gettok(%ip2c.mirc.ini.events,3,44)
  }
  if ((!%ip2c.mirc.ini.events.pos3) || (%ip2c.mirc.ini.events.pos3 == 0)) { 
    var %ip2c.mirc.ini.events = $readini($mircini,n,events,default)
    var %ip2c.mirc.ini.events.pos3 =  $calc($gettok(%ip2c.mirc.ini.events,3,44) + 1)
  }
  if (%ip2c.mirc.ini.events.pos3 == 1) { 
    return CHANNEL 
  }
  elseif (%ip2c.mirc.ini.events.pos3 == 2) { 
    return STATUS 
  }
  elseif (%ip2c.mirc.ini.events.pos3 == 3) { 
    return BOTH 
  }
  elseif (%ip2c.mirc.ini.events.pos3 == 4) { 
    return $false 
  }
  else { ip2c.echo.NotFound Quits display place has NO RESULT - programming error in ALIAS ip2c.read.mirc.ini.quit | return $false  }
}
...
dialog ip2cConfig {
  ...
  check "Long IP result"         16, 115  45  60 10, tab 1001
  check "Logging Not Found"      17, 115  55  60 10, tab 1001
  check "Echo Not Found msg"     18, 115  65  60 10, tab 1001
  check "Disable Check if Remote is On", 19, 115 75 90 10, tab 1001
  button "?"                    259, 180  45  10 10, tab 1001 ; Help button for Long IP Result
  button "?"                    256, 180  60  10 10, tab 1001 ; Help button for Logging/Not Found
  ...
  check "Request a feature"      24, 205  85  60 10, disable tab 1001
  button "?"                    260, 270  85  10 10,  tab 1001 ; Help button for any new feature


  box ": Help Box :"         211, 110  95 180 105, tab 1001
  edit "Help on Options",    212, 115 105 170  90, multi, vsbar, tab 1001
  ...
  box "Convert CSV tp a new Updated ip2contry.txt"          , 610,  15  35  200  40, tab 1003
  ...
  button "Location"               , 614,  25  80   30  10, tab 1003
  text " of new CSV File:"        , 615,  57  81   70  10, tab 1003
  text $scriptdir                 , 616, 100  81  150  10, tab 1003
  ...
  button "Get your downloaded CSV filenames" ,    311,  25   95  90  10, tab 1003
  list                           312,  25  110 200 60, sort, hsbar, vsbar, tab 1003
  button &Convert $+ $crlf $+ Selected $+ $crlf $+ File Now, 313,  245  120  40  40, center multi tab 1003
  edit "Conversion Progress: Not Started", 314,  125   95  160  10, autohs, disable tab 1003
  ...
  text "Reload old Data Now"          623, 225 45 70 10 center, hide, tab 1003
  button UN-load $+ $crlf $+ OLD Hash Table $+ $crlf $+ IMMEDIATELY !!, 315,  225   55  60  30, center, multi, hide, tab 1003
  button RE-load $+ $crlf $+ NEW Hash Table $+ $crlf $+ NOW !!        , 319,  225   55  60  30, center, multi, hide, tab 1003
  text Current file:            , 626, 25 170  55   10, tab 1003
  text Modification Time:       , 627, 25 180  55   10, tab 1003
  text Size of file:            , 628, 25 190  55   10, tab 1003
  edit                          , 316, 70 170  195  10, disable, tab 1003
  edit                          , 317, 70 180  195  10, disable, tab 1003
  edit                          , 318, 70 190  195  10, disable, tab 1003
  ...
  edit "Result - Engine Only"        , 424,  150 115 140 10, disable, tab 1004
  edit "Result - Full /ip2c Alias"   , 425,  150 125 140 10, disable, tab 1004
  edit "Result - Country Search"     , 426,  150 135 140 10, disable, tab 1004
...
ALIAS -l ip2c.Settings.Save {
  ...
  if (%ip2c.temp.delete.lookup.ini) {
    ...
    while (%l <= %lines) {
      ...
      remini -n %file %section %2nd.item
      inc %l 
    }
...
ALIAS ip2c.Dialog.HelpBox {
  ...
  elseif (%m == ipReply) {
    ...
    %e Location: Options - Allow !ip in Channek $crlf $crlf
  ...
  elseif (%m == PersonalSettings) {
    ...
    %e and modified 2nd Level Domains list are not what is wanted anymore. $crlf $crlf
  ...
  elseif (%m == UpdateDatabase) {
    ...
    %e The originaly Maxmind database file is downloaded from: $crlf
    %e http://geolite.maxmind.com/download/geoip/database/GeoIPCountryCSV.zip $crlf $crlf
    %e Once this file is received, it should be unzipped it into this script's directory: $crlf
    %e Example: $scriptdir $crlf $crlf
    %e Next, in the Update Database section, the newly unzipped file: $crlf
    %e GeoIPCountryWhois.csv must be found and selected. $crlf $crlf
    %e Next, the selected file is converted into this script's database $crlf
